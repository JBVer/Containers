...
/*   Updated: 2022/12/17 14:35:58 by jvermeer         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
public:
	...
	map(): _size(0), _cmp(key_compare()), _alloc(allocator_type())
	{
	...
	explicit map(const key_compare & comp, const Allocator & alloc = Allocator()): _size(0), _cmp(comp), _alloc(alloc)
	{
		_end = _alloc_node.allocate(1);
		_end->color = BLACK;
		_end->parent = NULL;
	...
	map(InputIt first, InputIt last, const key_compare & comp = key_compare(), const Allocator & alloc = Allocator()): _size(0), _cmp(comp), _alloc(alloc)
	{
		_end = _alloc_node.allocate(1);
		_end->color = BLACK;
		_end->parent = NULL;
	...
	map( const map& other ): _size(0), _cmp(other.key_comp()), _alloc(other.get_allocator())  
	{
		_end = _alloc_node.allocate(1);
		_end->color = BLACK;
		_end->parent = NULL;
	...
	T & operator[]( const Key& key )
	{
		...
		ret = insert(ft::make_pair(key, T())).first.base();
		return ret->pair->second;
	};
	...
	{
		node_pointer keyNode = search(pos->first);
		if (keyNode)
			eraseNode(keyNode);
	};
	...
	iterator lower_bound( const Key& key )
	{
		Compare lessfunc;
		iterator ret = begin();
		while (ret != end() && lessfunc(ret->first, key))
			++ret;
		return ret;
	};
	...
	iterator upper_bound( const Key& key )
	{
		Compare lessfunc;
		iterator ret = begin();
		while (ret != end() && !lessfunc(key, ret->first))
			++ret;
		return ret;
	};
...
private:
	...
	node_pointer search( const Key & key ) const
	{
		if (_root != _end)
		{
			...
			while (search != _end)
			{
				if (comp(key, search->pair->first))
					search = search->left;
				else if (comp(search->pair->first, key))
					search = search->right;
				else
					return search;
			}
	...
	void printNode(node_pointer node) const
	{
		...
		{
		std::cout << "Main	: key(" << node->pair->first << ") | T(" << node->pair->second << ") | color:" << node->color << std::endl;
		if (node->parent)
			std::cout << "Parent	: key(" << node->parent->pair->first << ") | T(" << node->parent->pair->second << ") | color:" << node->parent->color << std::endl;
		else
			std::cout << "Parent	: END" << std::endl;
		if (node->left != _end)
			std::cout << "Left	: key(" << node->left->pair->first << ") | T(" << node->left->pair->second << ") | color:" << node->left->color << std::endl;
		else
			std::cout << "Left	: END" << std::endl;
		if (node->right != _end)
			std::cout << "Right	: key(" << node->right->pair->first << ") | T(" << node->right->pair->second << ") | color:" << node->right->color << std::endl;
		else
